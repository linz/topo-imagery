    ...
    if data_type == DataType.AERIAL:
        ...
        standardising_args.append(
            StandardisingArgs(
                ...
                extra_args=["--cutline", os.path.join(test_data_dir, "cutline.fgb")],
            )
        )
    return standardising_args
...
def main() -> None:
    ...
    for arg in standardising_args:
        with tempfile.TemporaryDirectory() as tmp_path:
            ...
            subprocess.call(
                f"docker run  -v {tmp_path}:/tmp/ topo-imagery python3 standardise_validate.py \
                    --from-file {os.path.join(test_data_dir, arg.file)} --preset {arg.preset}  \
            ...
            for tile in tile_files:
                ...
                return_code = subprocess.call(
                    f"cmp --silent {os.path.join(tmp_path, output_file)} {os.path.join(test_data_dir, 'output',output_file)}",
                    shell=True,
                )
                if return_code != 0:

...
def test_return_list_of_all_files_if_force() -> None:
    ...
    processed = run_pdal_translate(source, concurrency=1, target_output=target, force=True)
    rmtree(target)
...
def test_return_list_of_changed_files() -> None:
    ...
    processed = run_pdal_translate(source, concurrency=1, target_output=target, force=False)
    rmtree(target)
